...
void signal(string dir, string file, string baselines, int usb, int pmt_board, string homedir);
int scanFiles(string inputDirectory);
int scanSignalFiles(vector<string> &fileList, string inputDirectory, int usb);

//////////////////////////////////////////////////////////////////////////////////////////////

void baselinemb(int disk_num)
{
  ...
  */
  ...
  for(int i=0;i<usbhowmanyboardscount;i++){
    ...
    if(structure[usb]==-2){
      ...
      string dir0 = "/e/h.0/localdev/readout/data1/OVDAQ/DATA/Run_" + runtmp + "/USB_" + to_string(usb);
...
void initializeusb(int usb, int pmt) {
  ...
  if(usemaroc2gainconstantsmb.compare("no")==0){
    ...
    for(int index = 1 ; index < 65; index++){
      com_usb(usb, pmt, index - 1, 2*gain[index]);  // applying gain constants from MySQL
    }
  }
                ...
                if(!res)
                {
                    ...
                    if(result)
                    {
                        ...
                        while(sql_row = mysql_fetch_row(result))
                        {
                            ...
                            pmtdata[usbtemp][pmttemp][7] = sql_row[9];     //op87
                            pmtdata[usbtemp][pmttemp][8] = sql_row[8];     //op75
...
//        }
        ...
        if(usbnum == boxusb)
            continue;
        string cmd = "/e/h.0/localdev/cpp_readout/decode/decode " + full_path + "binary/";
...
void generatebaseline(int usb, int pmt)
{
  ...
  string dir1 = "/e/h.0/localdev/readout/data1/OVDAQ/DATA/Run_" + runtmp + "/USB_" + to_string(usb)+ "/PMT_" + to_string(modulenum);
  
  string mkdir_cmd = "mkdir " + dir1;
  system(mkdir_cmd.c_str());
  
  string filebase = DataFolder + "Run_" + runtmp + "/binary/ ";
  

  string baselines_Path = "/e/h.0/localdev/cpp_readout/baselines/baselines ";

  string cmd_baseline = baselines_Path + filebase + "baseline_" + to_string(usb) + " " + to_string(pmt);
...
void checkeff(int usb,int pmt3)
{
    ...
    string file = localpath + "/signal_" + to_string(usb) + ".dec";
...
void process_signal(int usb)
{
    ...
    printf("Processing files and Producing Plots for USB=%d",usb);
    ...
    string cmd = "/e/h.0/localdev/cpp_readout/decode/decode ";
...
void generatecsv2(int usb, int pmt)
{
    ...
    signal(full_path,filename,baselines,usb,pmt,homedir);
...
//generate baselines
    string baselines_Path = "/e/h.0/localdev/cpp_readout/baselines/";
    ...
    filesize = scanSignalFiles(fileList,full_path + "/binary/",usb);
    string cmd = "/e/h.0/localdev/cpp_readout/decode/decode ";
    for(int i = 0;i<filesize; i++){
        string cmd1;
        cmd1 = cmd + full_path + "/binary/" + fileList[i] + " >> " + filename + ".dec";
        system(cmd1.c_str());
    }
    
    string temp1 = filename + ".dec";
    
    signal(full_path, temp1, baselines, usb, pmt, homedir);
}
...
void plotdatamb(int usb, int pmtmb)
{
    ...
    string runtmp(run_number);
    string full_path = "/e/h.0/localdev/readout/data1/OVDAQ/DATA/Run_" + runtmp + "/";
    string cmd = "/e/h.0/localdev/cpp_readout/histogram/histogram ";
...
void signal(string dir, string file, string baselines, int usb, int pmt_board, string homedir)
{
    ...
    int i,j,x;
    float f;
    double sig[65], sig_dev[65], base[65], base_dev[65], hist[65][2048], hist2[65][100], hits_pc[65], bins[100], photo[65], gain_pmt[65], mean_norm[64], rate_norm[65];
    string title[65];
...
nohomedir:
    ...
    //parse data file
    string cmd = "/e/h.0/localdev/cpp_readout/unpacksignal/unpacksignal " +  file + " " + to_string(pmt_board);
...
void initializeboard(string define_runnumber, int trigger_num, int pmtini, int pmtfin){
    ...
    for (pmt1 = pmtini; pmt1<=pmtfin; pmt1++){
        ...
        com_usb(usb_local, pmt_local, 85, pipedelay);           // no force trigger
...
void getavg(string sumfile, int usb, int pmtmb, int col)
{
    ...
    // read input file
    string full_path = "/e/h.0/localdev/readout/data1/OVDAQ/DATA/Run_" + runtmp + "/";
    string cmd = full_path + "USB_" + to_string(usb) + "/PMT_" + to_string(pmtmb) + "/" + sumfile;
    ...
    cout << "\n" << "Average " << colname <<" (USB= " << usb << ", pmt board=" << pmtmb << ") = " << average << " \n\n";
}
